(function() {
  ...
  async function renderPerformance() {
    try {
      ...
      // Build active engine list dynamically from trade data
      const activeEngLabels = [];
      if (won.filter(t=>t.type==="NEAR_CERTAIN").length + lost.filter(t=>t.type==="NEAR_CERTAIN").length > 0) activeEngLabels.push('NEAR-CERTAIN');
      if (won.filter(t=>t.type==="SHORT_TERM").length + lost.filter(t=>t.type==="SHORT_TERM").length > 0) activeEngLabels.push('WALLET COPY SHORT');
      if (won.filter(t=>t.type==="LONG_TERM").length + lost.filter(t=>t.type==="LONG_TERM").length + open.filter(t=>t.type==="LONG_TERM").length > 0) activeEngLabels.push('WALLET COPY LONG (suspended)');
      if (won.filter(t=>t.type==="ANTI_NC").length + lost.filter(t=>t.type==="ANTI_NC").length + open.filter(t=>t.type==="ANTI_NC").length > 0) activeEngLabels.push('ANTI-NC');
      if (won.filter(t=>t.type==="NEWS").length + lost.filter(t=>t.type==="NEWS").length + open.filter(t=>t.type==="NEWS").length > 0) activeEngLabels.push('NEWS');
      if (descEl) descEl.innerHTML = 'Live results · <b style="color:#00aa55">'+winRate+'% WR</b> · <b style="color:'+(totalPnl>=0?'#00aa55':'#dd3344')+'">'+(totalPnl>=0?'+':'')+'$'+Math.abs(totalPnl).toFixed(2)+' net</b> · Engines: '+activeEngLabels.join(' + ');
      ...
      // Sort by settlement time (closed_at), not entry time (timestamp) — equity curve must reflect when PnL was realized
      const settledByTime = [...won,...lost].sort((a,b)=>(a.closed_at||a.timestamp||'').localeCompare(b.closed_at||b.timestamp||''));
      let winStreak = 0;
      ...
      if (dailyLog && dailyLog.length > 1) {
        ...
        html += '<div style="font-size:0.68em;color:#555;margin-bottom:4px;font-weight:600">BANKROLL GROWTH — $'+(STARTING_BR+PAPER_TOPUP)+' invested → $'+bankroll.toFixed(0)+'</div>';
      ...
      // Per-trade equity curve — smoother than daily chart, shows every settlement
      // Per-trade equity curve: sort by settlement time not entry time
      const sortedSettled = [...won,...lost].sort((a,b)=>(a.closed_at||a.timestamp||'').localeCompare(b.closed_at||b.timestamp||''));
      if (sortedSettled.length > 5) {
        let cum = 0;
        const cumPts = sortedSettled.map(t=>{ cum += (t.pnl||0); return cum; });
        const minC = Math.min(0, ...cumPts);
        const maxC = Math.max(0, ...cumPts);
        const rangeC = (maxC - minC) || 1;
        const W2=420, H2=54, P2=6;
        const n = cumPts.length;
        const curve = cumPts.map((v,i)=>{
          const x = P2 + (i/(n-1||1))*(W2-P2*2);
          const y = H2 - P2 - ((v - minC)/rangeC)*(H2-P2*2);
          return x.toFixed(1)+','+y.toFixed(1);
        }).join(' ');
        const zeroYc = H2 - P2 - ((0 - minC)/rangeC)*(H2-P2*2);
        const lastColor = cumPts[cumPts.length-1] >= 0 ? '#00ff88' : '#ff4444';
        html += '<div style="margin-bottom:10px;background:#f2f2f2;border:1px solid #dddddd;border-radius:4px;padding:6px 10px">';
        html += '<div style="font-size:0.68em;color:#555;margin-bottom:4px;font-weight:600">P&amp;L EQUITY CURVE — '+n+' settled trades · per-trade resolution</div>';
        html += '<svg width="100%" viewBox="0 0 '+W2+' '+H2+'" style="display:block">';
        html += '<line x1="'+P2+'" y1="'+zeroYc.toFixed(1)+'" x2="'+(W2-P2)+'" y2="'+zeroYc.toFixed(1)+'" stroke="#aaaaaa" stroke-width="1" stroke-dasharray="3,2"/>';
        html += '<polyline points="'+curve+'" fill="none" stroke="'+lastColor+'" stroke-width="1.5" stroke-linejoin="round"/>';
        // Start and end labels
        const lastX = (P2 + (W2-P2*2)).toFixed(1);
        const lastY = (H2 - P2 - ((cumPts[n-1] - minC)/rangeC)*(H2-P2*2)).toFixed(1);
        const firstY = (H2 - P2 - ((cumPts[0] - minC)/rangeC)*(H2-P2*2)).toFixed(1);
        html += '<circle cx="'+P2+'" cy="'+firstY+'" r="3" fill="#888"/>';
        html += '<circle cx="'+lastX+'" cy="'+lastY+'" r="3" fill="'+lastColor+'"/>';
        html += '<text x="'+(parseFloat(lastX)+3)+'" y="'+lastY+'" font-size="8" fill="'+lastColor+'">'+(cumPts[n-1]>=0?'+':'')+'$'+cumPts[n-1].toFixed(0)+'</text>';
        html += '</svg></div>';
      }

      html += '<div style="font-size:0.72em;color:#555;margin-bottom:6px;font-weight:600">ACTIVE ENGINES</div>';
      html += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">';
      const ncW = won.filter(t=>t.type==="NEAR_CERTAIN").length, ncL = lost.filter(t=>t.type==="NEAR_CERTAIN").length;
      html += '<span style="background:#fafae0;color:#7a7000;padding:3px 8px;border-radius:3px;font-size:0.75em;font-weight:600">&#9679; NEAR-CERTAIN ('+ncW+'W/'+ncL+'L)</span>';
      ...
      const newsW = won.filter(t=>t.type==="NEWS").length, newsL = lost.filter(t=>t.type==="NEWS").length, newsO = open.filter(t=>t.type==="NEWS").length;
      if (newsW+newsL+newsO > 0) html += '<span style="background:#fff5ee;color:#cc5500;padding:3px 8px;border-radius:3px;font-size:0.75em;font-weight:600">&#9679; NEWS SIGNAL ('+newsW+'W/'+newsL+'L · '+newsO+' open)</span>';
      ...
      if (arbWon+arbLost+arbOpen > 0) html += '<span style="background:#fff3e0;color:#b35a00;padding:3px 8px;border-radius:3px;font-size:0.75em;font-weight:600">&#9679; ARB KALSHI/PM ('+arbWon+'W/'+arbLost+'L · '+arbOpen+' open)</span>';
      html += '</div>';
  ...
  async function renderPaperTrades() {
    ...
    try {
      ...
      // Trade table — uses global engLabel()/engColor() as single source of truth
      // (removed local engineLabel duplicate that had wrong colors and missing engines)
      const engineLabel = t => {
